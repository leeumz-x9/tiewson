// TiewSonAI.js - Fixed Speech Recognition Issue
...
const shouldStopWakeWord = useRef(false); // ✅ เพิ่ม flag ป้องกัน auto-restart
...
useEffect(() => {
    ...
    try {
        ...
        wakeWord.onend = () => {
            ...
            // ✅ Auto-restart เฉพาะเมื่อไม่อยู่ในโหมดแชท
            if (!shouldStopWakeWord.current && !isExpanded) {
                ...
                wakeWordRestartTimeoutRef.current = setTimeout(() => {
                    if (!shouldStopWakeWord.current && !isWakeWordActive.current && !isExpanded) {
                        try {
                            console.log('🔄 Auto-restarting wake word');
                            wakeWord.start();
                        } catch (e) {
                            if (!e.message.includes('already started')) {
                                console.warn('Wake word restart failed:', e.message);
                            }
                        }
                    }
                }, 1500);
        ...
        wakeWord.onresult = (event) => {
            ...
            if (WAKE_WORDS.some(word => transcript.includes(word))) {
                ...
                // ✅ หยุด wake word และป้องกัน auto-restart
                shouldStopWakeWord.current = true;
                wakeWord.stop();
        ...
        wakeWord.onerror = (e) => {
            ...
            if (!shouldStopWakeWord.current && !isExpanded) {
                ...
                wakeWordRestartTimeoutRef.current = setTimeout(() => {
                    if (!shouldStopWakeWord.current && !isWakeWordActive.current) {
                        try {
    ...
    try {
        ...
        chatRecognition.continuous = true; // ✅ เปลี่ยนเป็น true เพื่อไม่ให้หลุด
        chatRecognition.interimResults = true; // ✅ เพิ่ม interim results
        ...
        chatRecognition.onend = () => {
            ...
            console.log('🛑 Chat listening ended');
        };

        chatRecognition.onresult = (event) => {
            // ✅ ใช้ final result เท่านั้น
        ...
        chatRecognition.onerror = (e) => {
            console.warn('Chat recognition error:', e.error);
...
}, [language, speak, stopSpeaking, isExpanded, handleSendMessage]);

// ========================== WAKE WORD CONTROL ==========================
useEffect(() => {
    const manageWakeWord = () => {
        if (isExpanded || isListening) {
            // ✅ หยุด wake word เมื่อเข้าโหมดแชท
            shouldStopWakeWord.current = true;
            ...
            // ✅ เปิด wake word เมื่อออกจากโหมดแชท
            shouldStopWakeWord.current = false;
...
const toggleListening = useCallback(() => {
    if (isListening) {
        // ✅ หยุดฟัง
        console.log('🛑 Manually stopping chat recognition');
        recognitionRef.current?.stop();
        setIsListening(false);
        isRecognitionActive.current = false;
    } else {
        ...
        setTimeout(() => {
            try {
                ...
                if (recognitionRef.current && !isRecognitionActive.current) {
                    ...
                    console.log('▶️ Starting chat recognition');
    ...
    <>
        ...
        {!isExpanded && (
            <div className="fixed bottom-6 right-6 z-40">
                <button
                    onClick={() => {
                        shouldStopWakeWord.current = true; // ✅ หยุด wake word
                        wakeWordRecognitionRef.current?.stop();
                ...
                >
                    <img
                        src="/tiewson.png"
                        alt="น้องทิวสน"
                        className={`w-32 h-32 object-cover rounded-full shadow-2xl border-4 border-white transition-all duration-300 ${
                ...
                <div className="w-full md:w-96 bg-white rounded-3xl shadow-2xl overflow-hidden border-2 border-blue-100 mb-4">
                    ...
                    <div className="h-80 overflow-y-auto p-4 bg-gray-50">
                        ...
                        {messages.map((msg, idx) => (
                            ...
                            >
                                <div
                                    className={`inline-block max-w-[85%] p-3 rounded-2xl whitespace-pre-wrap text-sm md:text-base ${
                                        msg.role === 'user'
